...
fig, (ax_time, ax_mesh) = plt.subplots(nrows=2, gridspec_kw={
    'height_ratios': [1, 4]}, constrained_layout=True)
xx, yy = np.meshgrid(lons, lats)
vmin, vmax = 1e13, 8e13
norm = matplotlib.colors.Normalize(vmin, vmax)
mesh = ax_mesh.pcolormesh(xx, yy, output[0, :, :, i].T,
                          shading='auto', norm=norm)
plt.colorbar(mesh, label=f'N number density at {alt} km (/m$^3$)',
...
ani = FuncAnimation(fig, update_surface, frames=range(ndates),
                    interval=25)
