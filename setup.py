...
ext_msis20 = Extension(
    name="pymsis.msis20f",
    sources=[
        str(x) for x in msis20_wrappers + msis20_sources
    ],  # [str(x) for x in msis2_wrappers + msis20_sources],
...
ext_msis21 = Extension(
    name="pymsis.msis21f",
    sources=[str(x) for x in msis21_wrappers + msis21_sources],
    extra_f90_compile_args=["-std=legacy", "-O1"],
)

msis00_sources = [Path("src/wrappers/msis00.F90"), Path("src/msis00/NRLMSISE-00.FOR")]
ext_msis00 = Extension(
    name="pymsis.msis00f",
...
setup(
    ...
    name="pymsis",
    data_files=[
        ("pymsis", [str(Path("pymsis/msis2.0.parm")), str(Path("pymsis/msis21.parm"))])
    ],
