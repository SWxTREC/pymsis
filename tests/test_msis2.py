...
def test_output():
    return np.array([2.465493e-10, 2.927530e+15, 1.251696e+14, 3.845063e+15,
                     ...
                     3.302753e-04, 9.999000e-38, 9.845342e+02],
                    dtype=np.float32)
...
def test_run_options():
    ...
    assert_array_equal(msis2.run(*test_data(), options=None),
                       msis2.run(*test_data(), options=[1]*25))

    with pytest.raises(ValueError, match='options needs to be a list'):
        msis2.run(*test_data(), options=[1]*22)


def test_run_single_point():
    output = msis2.run(*test_data())
...
def test_run_wrapped_lon():
    ...
    output1 = msis2.run(*input_data)
    input_data = (date, 180, lat, alt, f107, f107a, ap)
    output2 = msis2.run(*input_data)
